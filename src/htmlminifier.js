...
function unwrapMediaQuery(text) {
  const matches = text.match(/^@media ([\s\S]*?)\s*{[\s\S]*}$/);
  return matches ? matches[1] : text;
}
...
function minify(value, options, partialMarkup) {
  ...
  function escapeFragments(fn) {
    return (text, type) => fn(text.replace(uidPattern, (match, prefix, index) => {
  ...
  let str = joinResultSegments(buffer, options);

  if (uidPattern) {
    str = str.replace(uidPattern, (match, prefix, index, suffix) => {
  ...
  if (uidIgnore) {
    str = str.replace(new RegExp(`<!--${uidIgnore}([0-9]+)-->`, 'g'), (match, index) => ignoredMarkupChunks[Number(index)]);
  }
